...
app.use((req, res, next) => {
  if (req.originalUrl.split('?')[0] === '/api/webhooks/razorpay') {
    return next();
  }
  ...
  function orderStatusAccess(req, res, next) {
    authRequired(req, res, () => {
      if (!['admin', 'staff', 'chef'].includes(req.user.role)) {
        return res.status(403).json({ detail: 'Order status access only' });
      }
      next();
    });
  }
...
app.post('/api/pos/orders/:id/pay', posAccess, async (req, res) => {
  ...
  if (!['cash', 'upi', 'card'].includes(method)) {
    return res.status(400).json({
      detail: 'Collection method must be cash, upi, or card',
    });
  }

  if (!Number.isFinite(amount) || amount <= 0) {
    return res.status(400).json({
      detail: 'A valid collected amount is required',
    });
  }
  ...
  if (['cancelled', 'delivered'].includes(order.status)) {
    return res.status(400).json({
      detail: 'Cannot collect payment for a cancelled or delivered order',
    });
  }

  if (roundMoney(amount) !== roundMoney(order.total)) {
    return res.status(400).json({
      detail: 'Collected amount must match the order total',
    });
  }
  ...
  if (!updated) {
    return res.status(409).json({
      detail: 'Payment state changed; refresh and retry',
    });
  }
...
app.post('/api/webhooks/razorpay', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  // Reject duplicate status requests.
  ...
  // Validate the requested status transition and user role.
  const validation = transitionOrderStatus(
    current,
    requestedStatus,
    req.user.role
  );
  ...
  // Never mark an order as delivered until payment is confirmed.
  if (
    requestedStatus === 'delivered' &&
    current.payment_status !== 'paid'
  ) {
    return res.status(400).json({
      detail: 'Payment must be confirmed before marking the order as delivered.',
    });
  }

  // Update ONLY the order status. Payment status remains untouched.
