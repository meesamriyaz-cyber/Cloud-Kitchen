  ...
  return (
    <div className="max-w-md mx-auto px-5 py-14">
      ...
      <form onSubmit={submit} className="space-y-4">
        <div>
          ...
          <Input id="email" type="email" required value={email} onChange={(e) => setEmail(e.target.value)}
            className="h-11 rounded-xl mt-1" data-testid="login-email-input" />
        </div>
        <div>
          <Label htmlFor="password">Password</Label>
          <Input id="password" type="password" required value={password} onChange={(e) => setPassword(e.target.value)}
            className="h-11 rounded-xl mt-1" data-testid="login-password-input" />
        </div>
      ...
      <p className="text-sm text-stone-500 mt-6 text-center">
        No account? <Link to="/signup" className="text-[#E76F51] font-medium hover:underline">Sign up</Link>
      </p>
    </div>
  );
}
