      ...
      {order.status === "cancelled" ? (
        ...
        <div className="mt-10 soft-panel p-6">
              ...
              return (
                <div key={s.key} className="flex flex-col items-center text-center" data-testid={`step-${s.key}`}>
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors
                    ${done ? "bg-orange-600 text-white" : "bg-stone-100 text-stone-400"}
                    ${active ? "ring-4 ring-orange-700/20" : ""}`}>
                    <Icon size={16} />
                  </div>
          ...
          <div className="mt-6 relative h-1.5 bg-stone-100 rounded-full overflow-hidden">
            <div className="absolute inset-y-0 left-0 bg-orange-600 transition-all duration-500"
      ...
      <div className="mt-8 grid md:grid-cols-2 gap-5">
        <div className="soft-panel p-5">
          ...
          <div className="mt-4 pt-3 border-t border-stone-200 space-y-1 text-sm text-stone-600">
            ...
            {order.discount > 0 && <div className="flex justify-between text-green-700"><span>Discount</span><span>-{formatMoney(order.discount)}</span></div>}
            ...
            <div className="flex justify-between font-bold text-stone-900 text-base pt-2 border-t border-stone-200"><span>Total</span><span>{formatMoney(order.total)}</span></div>
          </div>
        </div>
        <div className="soft-panel p-5">
          <h3 className="font-display font-semibold mb-3">{order.channel === "pos" ? "Customer" : "Delivery to"}</h3>
          <div className="text-sm text-stone-700 space-y-1">
            <div className="font-medium">{orderCustomer(order)}</div>
          ...
          <div className="text-sm mt-1 capitalize">{order.payment_method === "cod" ? "Cash on Delivery" : `${order.payment_method} - ${order.payment_status}`}</div>
        </div>
      </div>
    </div>
  );
